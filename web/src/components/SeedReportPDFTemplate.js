...
const SeedReportPDFTemplate = React.forwardRef(({ report }, ref) => {
    if (!report || !report.matrix_results) return null;

    const results = report.matrix_results;
    ...
    return (
        <div ref={ref} style={{
            ...
            {/* RESULTADOS DA ANÁLISE DE SEMENTES */}
            <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
                <h2 style={{ fontSize: '18px', color: '#15803d', fontWeight: 'bold', margin: '8px 0', paddingLeft: '5px', textAlign: 'center', textTransform: 'uppercase', letterSpacing: '0.5px' }}>
                    Resultados para Pesquisa e Quantificação de Fungos
                </h2>

                {/* Suporte a novo formato (samples[]) e legado (objeto flat) */}
                {(results.samples || [results]).map((sample, sIndex) => {
                    const filterEmpty = (arr = []) => {
                        const f = arr.filter(i => i.genus && i.genus.trim() !== '');
                        return f.length > 0 ? f : null;
                    };
                    const pathogenicList = filterEmpty(sample?.pathogenic);
                    const deterioratingList = filterEmpty(sample?.deteriorating);
                    const contaminatingList = filterEmpty(sample?.contaminating);

                    return (
                        <div key={sIndex} style={{ background: '#fff', borderRadius: '12px', boxShadow: '0 4px 15px rgba(0,0,0,0.02)', pageBreakInside: 'avoid', breakInside: 'avoid', border: '1px solid #dcfce7', overflow: 'hidden' }}>
                            {/* Identificação / Técnica */}
                            {sample?.identification && (
                                <div style={{ background: '#f0fdf4', borderBottom: '1px solid #dcfce7', padding: '14px 20px', display: 'flex', justifyContent: 'space-between', gap: '20px', flexWrap: 'wrap' }}>
                                    <div>
                                        <span style={{ display: 'block', fontSize: '10px', color: '#6b7280', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: '4px' }}>Identificação</span>
                                        <strong style={{ fontSize: '13px', color: '#15803d' }}>{sample.identification}</strong>
                                    </div>
                                    {sample?.analytical_technique && (
                                        <div style={{ textAlign: 'right' }}>
                                            <span style={{ display: 'block', fontSize: '10px', color: '#6b7280', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: '4px' }}>Técnica Analítica</span>
                                            <strong style={{ fontSize: '13px', color: '#1a1a1a' }}>{sample.analytical_technique}</strong>
                                        </div>
                                    )}
                                </div>
                            )}
                            ...
                            <div style={{ padding: '16px 20px', display: 'flex', flexDirection: 'column', gap: '10px' }}>
                                {/* Patogênicos */}
                                {pathogenicList && (
                                    <div style={{ borderLeft: '4px solid #16a34a', paddingLeft: '16px', borderRadius: '4px' }}>
                                        <h4 style={{ margin: '0 0 10px 0', color: '#15803d', fontSize: '11px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.5px' }}>Gêneros Patogênicos</h4>
                                        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                                            {pathogenicList.map((item, idx) => (
                                                <div key={idx} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingBottom: '8px', borderBottom: '1px solid rgba(0,0,0,0.04)' }}>
                                                    <span style={{ fontSize: '13px', color: '#1a1a1a', fontWeight: 500, fontStyle: 'italic' }}>{item.genus}</span>
                                                    <span style={{ fontSize: '12px', backgroundColor: '#bbf7d0', color: '#15803d', padding: '3px 12px', borderRadius: '16px', fontWeight: 700 }}>{item.percent}</span>
                                                </div>
                                            ))}
                                        </div>
                                    </div>
                                ...
                                {/* Deteriorantes */}
                                {deterioratingList && (
                                    <div style={{ borderLeft: '4px solid #4ade80', paddingLeft: '16px', borderRadius: '4px' }}>
                                        <h4 style={{ margin: '0 0 10px 0', color: '#15803d', fontSize: '11px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.5px' }}>Deteriorante / Armazenamento</h4>
                                        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                                            {deterioratingList.map((item, idx) => (
                                                <div key={idx} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingBottom: '8px', borderBottom: '1px solid rgba(0,0,0,0.04)' }}>
                                                    <span style={{ fontSize: '13px', color: '#1a1a1a', fontWeight: 500, fontStyle: 'italic' }}>{item.genus}</span>
                                                    <span style={{ fontSize: '12px', backgroundColor: '#bbf7d0', color: '#15803d', padding: '3px 12px', borderRadius: '16px', fontWeight: 700 }}>{item.percent}</span>
                                                </div>
                                ...
                                {/* Contaminantes */}
                                {contaminatingList && (
                                    <div style={{ borderLeft: '4px solid #86efac', paddingLeft: '16px', borderRadius: '4px' }}>
                                        <h4 style={{ margin: '0 0 10px 0', color: '#15803d', fontSize: '11px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.5px' }}>Contaminante</h4>
                                        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                                            {contaminatingList.map((item, idx) => (
                                                <div key={idx} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingBottom: '8px', borderBottom: '1px solid rgba(0,0,0,0.04)' }}>
                                                    <span style={{ fontSize: '13px', color: '#1a1a1a', fontWeight: 500, fontStyle: 'italic' }}>{item.genus}</span>
                                                    <span style={{ fontSize: '12px', backgroundColor: '#bbf7d0', color: '#15803d', padding: '3px 12px', borderRadius: '16px', fontWeight: 700 }}>{item.percent}</span>
                                                </div>
...
SeedReportPDFTemplate.displayName = 'SeedReportPDFTemplate';

export default SeedReportPDFTemplate;
